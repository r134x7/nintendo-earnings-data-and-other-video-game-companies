...
export default function GRAPH_NINTENDO_MST_FY3_22() {
    ...
    // const graphQuartersRegionC = differenceTitles.flatMap((elem) => {
    //     return elem.map((secondElem, secondIndex, array) => {
    //         return (secondIndex === 0)
    //                 ? array.reduce((prev, next) => {
    //                     return prev + next.valueC
    //                 }, 0)
    //                 : 0
    //     }).filter((elem) => elem !== 0)
    // })

    // const overseasMax = differenceTitles.flatMap((elem) => {
    //     return elem.map((secondElem, secondIndex, array) => {
    //         return (secondIndex === 0)
    //                 ? array.reduce((prev, next) => {
    //                     return prev + next.valueB
    //                 }, 0)
    //                 : 0
    //     }).filter((elem) => elem !== 0)
    // })
