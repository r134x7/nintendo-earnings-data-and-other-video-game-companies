...
#Net Sales (by quarter), variable creates an array
ns1 = [ 322647, 624272, 1320219, 1320219, 1600000, 1600000, 1600000,  1650000, 0  ] #input figures, last two figures are forecasts, 1st, 2nd and 3rd forecast revisions
#+++++++++++++++++++++++++++++
ns2 = [ ns1[0], ns1[1]  - ns1[0], ns1[2] - ns1[1], ns1[3] - ns1[2], ns1[4], ns1[5], ns1[6], ns1[7], ns1[8]] #quarterly calculation, forecasts


ns3 = [ '¥{:,}M '.format(elem) for elem in ns2 ] #format all integers to string to add ¥ and M

ns4 = [ '{0: >13}'.format(elem) for elem in ns3]
#format width

nsfy = [ ns2[0] + ns2[1] + ns2[2] +ns2[3], ns2[0] + ns2[1], ns2[0] + ns2[1] + ns2[2] ] # fiscal year cumulative, 1st half, 1st 3 quarters

nsfy1 = [ '¥{:,}M '.format(elem) for elem in nsfy ] #format all integers to string to add ¥ and M

nsfy2 = [ '{0: >13}'.format(elem) for elem in nsfy1]
#format width

#last years numbers
nsly1= [ 358106, 411418, 634939, 354447, 1758910 ] #Q1-4, fy.

nsly2 = [ ((ns2[0] / nsly1[0]) - 1) * 100, ((ns2[1] / nsly1[1]) -1) *100, ((ns2[2] / nsly1[2]) - 1) * 100,  ((ns2[3] / nsly1[3]) - 1) * 100]
...
ns_yoyfy1 = [  (((ns2[0] + ns2[1]) / (nsly1[0] + nsly1[1])) - 1) * 100, (((ns2[0] + ns2[1] + ns2[2]) / (nsly1[0] + nsly1[1] + nsly1[2])) - 1) * 100, ((nsfy[0] / nsly1[4]) - 1) * 100] #Q2-4 fy cumulative
...
op1 = [ 119752, 219959, 472551, 472551, 500000, 520000, 520000,  560000, 0  ] #input figures, last two are forecasts
#+++++++++++++++++++++++++++++
op2 = [ op1[0], op1[1] - op1[0], op1[2] - op1[1], op1[3] - op1[2], op1[4], op1[5], op1[6], op1[7], op1[8]]

op3 = [ '¥{:,}M '.format(elem) for elem in op2 ] #format all integers to string to add ¥ and M

op4 =  [ '{0: >18}'.format(elem) for elem in op3]
#format width

opfy = [op2[0] + op2[1] + op2[2] + op2[3], op2[0] + op2[1], op2[0] + op2[1] + op2[2] ]

opfy1 = [ '¥{:,}M '.format(elem) for elem in opfy ] #format all integers to string to add ¥ and M

opfy2 =  [ '{0: >18}'.format(elem) for elem in opfy1]
#format width

#last years numbers
oply1= [ 144737, 146687, 229684, 119526, 640634 ] #Q1-4, fy.

oply2 = [ ((op2[0] / oply1[0]) - 1) * 100, ((op2[1] / oply1[1]) -1) *100, ((op2[2] / oply1[2]) - 1) * 100,  ((op2[3] / oply1[3]) - 1) * 100]
...
op_yoyfy1 = [  (((op2[0] + op2[1]) / (oply1[0] + oply1[1])) - 1) * 100, (((op2[0] + op2[1] + op2[2]) / (oply1[0] + oply1[1] + oply1[2])) - 1) * 100, ((opfy[0] / oply1[4]) - 1) * 100] #Q2-4 fy cumulative
...
op_mgxfynextfc = ((op1[4] / ns1[4]) ) * 100
op_mgxfynextfc1 =  '{:.2f}% '.format(op_mgxfynextfc) 
op_mgxfynextfc2 =  '{0: >12}'.format(op_mgxfynextfc1)

#Net Income (by quarter)
np1 = [ 92747, 171834, 367387, 367387, 340000, 350000,  350000,  400000, 0 ] #input figures, last two are forecasts
#+++++++++++++++++++++++++++++
np2 = [ np1[0], np1[1] - np1[0], np1[2] - np1[1], np1[3] - np1[2],  np1[4], np1[5], np1[6], np1[7], np1[8]]

np3 = [ '¥{:,}M '.format(elem) for elem in np2 ]

np4 =  [ '{0: >12}'.format(elem) for elem in np3]
#format width
...
npfy2 =  [ '{0: >12}'.format(elem) for elem in npfy1]
#format width
