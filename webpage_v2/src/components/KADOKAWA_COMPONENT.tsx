...
import { kadokawaSegmentEarningsList } from "../data/generalTables/segment_earnings_general";
import { kadokawaConsolidatedEarningsList, kadokawaConsolidatedEarningsGraphList } from "../data/generalTables/consolidated_earnings_general";
import { kadokawaLinks } from "../data/generalTables/data_sources_general";
...
export default function KADOKAWA_COMPONENT(props: {setIndex: number; yearLength: number}) {
    ...
    const componentListNew = Array.from({length: props.yearLength}, (elem, index) => {
        ...
        return [
            { 
                name: "Data Sources",
                value: kadokawaLinks.get(index),
            },
            {
                name: "Consolidated Operating Results",
                value: kadokawaConsolidatedEarningsList.get(index),
                graph: <GRAPH_CONSOLIDATED_EARNINGS setData={kadokawaConsolidatedEarningsGraphList.get(index)} />
            },
            {
                name: "Segment Information",
                value: kadokawaSegmentEarningsList.get(index),
            },
