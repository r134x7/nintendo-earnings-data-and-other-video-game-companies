...
const readQuarter = (currentQuarterLocal) => {
    ...
    return (currentQuarterLocal = 1)
            ? fs.readFileSync("firstQuarter.html", "utf-8")
            : (currentQuarterLocal = 2)
            ? fs.readFileSync("secondQuarter.html", "utf-8") 
            : (currentQuarterLocal = 3)
            ? fs.readFileSync("thirdQuarter.html", "utf-8")
            : fs.readFileSync("fourthQuarter.html", "utf-8");
};
...
const getJSON = (jsonLocal, currentQuarterLocal) => {
    ...
    return (currentQuarterLocal > 1)
        ? fs.readFileSync(jsonLocal, "utf-8")
        : undefined
}; 
