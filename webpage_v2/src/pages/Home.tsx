...
const Home = () => {
    ...
    const capcomStatus = liner(printTextBlock("Capcom page status",32),"=","top",true,32) + liner(printTextBlock("Latest update: Consolidated Financial Results in Special Page has been updated with data from all quarters. Consolidated Financial Results from 1998 to 2023 is now in the Date By Fiscal Year section.",40),"−","both",true,40) + liner(printTextBlock("Future update planned: Notes section.",40),"−","bottom",true,40);
    ...
    const eventsStatus = liner(printTextBlock("Events page status",32),"=","top",true,32) + liner(printTextBlock("Page update: Updated calendar for 14 of 22 companies from July to August 2023.",40),"−","both",true,40);
